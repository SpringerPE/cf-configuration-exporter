...
class TerraformMutation(ManifestMutation):
    ...
    def mutate_manifest(self):
        ...
        tf_manifest["cf_security_groups"] = self.mutate_security_groups(
            self.cf_dict)

        logging.info("Mutating flags...")
        tf_manifest["cf_feature_flags"] = self.mutate_feature_flags(
            self.cf_dict)
    ...
    def mutate_feature_flags(self, cf_dict):
        ...
        flags_to_port = [
            flag for flag in cf_flags if flag['name'] in supported_flags]
...
class CFConfiguratorMutation(ManifestMutation):
    ...
    def mutate_manifest(self):
        ...
        logging.info("Mutating security_groups...")
        cc_manifest["cf_security_groups"] = self.mutate_security_groups(
            self.cf_dict)

        logging.info("Mutating flags...")
        cc_manifest["cf_feature_flags"] = self.mutate_feature_flags(
            self.cf_dict)
